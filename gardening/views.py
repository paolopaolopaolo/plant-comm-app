...
class GreenThumbPage(View):
	...
	def RETURN_USER_DATA(self, _id = None):
		if _id is None:
...
class ProfilePage(GreenThumbPage, APIView):
	...
	def get(self, request, *args, **kwargs):
		...
		self.context['plant_objects'] = json.dumps(sorted(self.RETURN_PLANT_DATA(_id = __user),
										 key=self.sortById))
		self.context['gardener_object'] = json.dumps(self.RETURN_USER_DATA(_id = __user))

		self.context['plant_img_objects'] = json.dumps(self.RETURN_PLANT_DATA(img = True, _id = __user))
